...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadFeed() {
    ...
    posts.forEach((p) => {
      ...
      name.textContent =
        p.username || p.user_address.slice(0, 6) + "..." + p.user_address.slice(-4);
    ...
    if (currentAddress && myPostsEl) {
      const mine = posts.filter(
        (p) => p.user_address.toLowerCase() === currentAddress.toLowerCase()
      );
      myPostsEl.innerHTML = "";
      mine.forEach((p) => {
  ...
  if (uploadBtn) {
    uploadBtn.addEventListener("click", async () => {
      ...
      if (cfConf.configured) {
        publicUrl =
          cfConf.deliveryUrl.replace(/\/+$/, "") + "/" + upData.result.id + "/public";
  ...
  avatarSend.addEventListener("click", async () => {
    ...
    if (cfConf.configured) {
      avatarUrl =
        cfConf.deliveryUrl.replace(/\/+$/, "") + "/" + upData.result.id + "/avatar";
    } else if (upData.result.variants && upData.result.variants.length) {
