...
async function connectWallet() {
  ...
  if (chainId !== "0x171") {
    try {
      ...
      alert("Troque para a PulseChain chainId 369");
    }
  }
  ...
  posts.forEach(p => {
    ...
    card.innerHTML = `
      ...
      <div class="post-media">
        ${p.media_type.startsWith("video") ? `<video src="${p.media_url}" controls></video>` : `<img src="${p.media_url}" loading="lazy">`}
      </div>
...
async function loadMyPosts() {
  ...
  mine.forEach(p => {
    ...
    item.innerHTML = `
      ${p.media_type.startsWith("video") ? `<video src="${p.media_url}" controls></video>` : `<img src="${p.media_url}">`}
      <div class="post-caption">${p.caption || ""}</div>
    `;
...
doUpload.addEventListener("click", async () => {
  ...
  const preset = document.getElementById("cloud-preset").value;
  const cloud = document.getElementById("cloud-name").value;
  if (!fileInput.files.length) {
    alert("Select file");
    return;
  }
  if (!preset || !cloud) {
    alert("Fill Cloudinary data");
    return;
  }
  ...
  const resp = await fetch(`https://api.cloudinary.com/v1_1/${cloud}/upload`, {
    method:"POST",
    body:formData
  });
